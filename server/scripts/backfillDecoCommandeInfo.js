/**
 * Script ponctuel : peuple Deco.dateCommande/codeClient/refClient/nombreProfil/nombreKitPose/
 * formatPlaqueGamesys (données de commande Gamesys, fd_entete_devi/fd_dossier) pour les documents
 * existants qui ne les ont pas encore (créés avant l'ajout de ces champs, ou avant la mise en place
 * de la synchro proactive des stubs).
 *
 * Usage :
 *   node server/scripts/backfillDecoCommandeInfo.js            (dry-run)
 *   node server/scripts/backfillDecoCommandeInfo.js --apply     (exécution réelle)
 *
...
const { backfillDecoCommandeInfo } = require("../src/services/decoCommandeInfoBackfillService");
...
async function main() {
  ...
  const resume = await backfillDecoCommandeInfo({ dryRun: !apply });
