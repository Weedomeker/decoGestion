/**
 * Script ponctuel : peuple Deco.prixTotal (somme Gamesys fd_entete_devi.endv_px_total sur tous les
 * sous-dossiers d'une commande racine) pour les documents existants qui ne l'ont pas encore
 * (créés avant l'ajout de ce champ).
 *
 * Usage :
 *   node server/scripts/backfillDecoPrix.js            (dry-run)
 *   node server/scripts/backfillDecoPrix.js --apply     (exécution réelle)
 *
...
const { backfillDecoPrix } = require("../src/services/decoPrixBackfillService");
...
async function main() {
  ...
  const resume = await backfillDecoPrix({ dryRun: !apply });

  console.log("\nRésumé :");
  console.log(`  Documents sans prixTotal : ${resume.candidats}`);
  if (apply) {
