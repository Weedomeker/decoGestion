...
async function addSvgToPdf(pdfPath, svgContent, outputPath) {
  try {
    ...
    console.log('SVG superposé avec succès sur le PDF');
  } catch (error) {
    console.error('Erreur lors de la superposition du SVG :', error.message);
  }
}
...
async function createPdfFromSvg(svgContent, pdfDoc) {
  return new Promise((resolve, reject) => {
    ...
    doc.addPage({ size: [cmToPoints(215), cmToPoints(100)] });
...
const pdfPath = './test/DIBOND 101X215-TEST_JOHN 100x210_S_.pdf'; // Chemin vers le fichier PDF existant
const svgContent = fs.readFileSync('./test/100x210.svg', 'utf8'); // Lire le contenu SVG
const outputPath = './test/preview.pdf'; // Chemin pour le fichier de sortie
